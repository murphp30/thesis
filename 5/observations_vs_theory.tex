...
Burst Time & $I_0$  & $x_0$ & $y_0$  & FWHM\textsubscript{x} & FWHM\textsubscript{y} & $\theta$ \\
 & (arbitrary) & (arcsec) & (arcsec) & (arcmin) & (arcmin) &  (degrees) \\
\midrule
...
Perhaps a more damning indictment for all scattering models comes when one relaxes the assumption of a spherically symmetric solar corona. The observations in this study were made over the course of a number of days. If, as is widely accepted, radio burst generation is concentrated above an evolving active region then spectrum of density inhomogeneities will surely be different from one day to another. The time scale of this change is an important parameter that is not considered in scattering models nor is it understood by the author
