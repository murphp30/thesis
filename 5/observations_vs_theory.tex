...
In order to deterime if the fit a 2D Gaussian in visibility space is appropriate, a Markov Chain Monte Carlo (MCMC) method was employed. MCMC samples a given distribution, in this case the model parameters. By constantly updating in a chain the entire distribution is sampled and a most probable estimate is obtained. The corner plot in Figure \ref{fig:corner} shows the individual histograms as well as the 2D correlations between various parameters. This gives an excellent and robust measure of the error in each parameter.
