...
\end{align}
\end{subequations}
Here $N$ is the electron density, $\mathbf{u}$ is the fluid velocity, $m$ is the particle mass, $\mathbf{B}$ is the magnetic field strength, $p$ is the pressure, $\gamma$ is the adiabatic index and $T$ is the temperature.
The assumption that each particle species is an isothermal gas with temperature $T$ gives $p_0 = N_0 k_b T$. Maxwell's equations then become
...
\section{Scattering of radio waves in the solar corona}
